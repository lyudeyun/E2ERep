...
# Fast version: Remove 0.8 scaling, use 1280x720 directly from CARLA
inference_only_pipeline = [
    ...
    dict(
        type='MultiScaleFlipAug3D',
        img_scale=(1280, 720),  # Fast version: Direct 1280x720 from CARLA, no scaling
        pts_scale_ratio=1,
        flip=False,
        transforms=[
            # Removed RandomScaleImageMultiViewImage - images are already 1280x720 from CARLA
            dict(type='PadMultiViewImage', size_divisor=32),
            dict(type='VADFormatBundle3D', class_names=class_names, with_label=False, with_ego=True),
